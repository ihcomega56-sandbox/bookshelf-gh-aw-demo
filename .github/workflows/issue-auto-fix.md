---
description: |
  Issue に `refactoring` / `documentation` / `bug` のいずれかのラベルが付与されたときに、
...
トリガーとなった Issue（ラベル: `refactoring` / `documentation` / `bug` のいずれか）を読み、
自動で最小限の修正を行いドラフトのプルリクエストを作成してください。

## 手順

1. Issue のタイトル・本文・既存のコメント（トリアージ結果など）を読み、要求内容を正確に把握する。
2. 付与されているラベルに応じて対応方針を切り替える。
   - `bug`: 再現手順・期待する動作・実際の動作を確認し、`src/main/java` 配下の該当コードを修正する。
     既存テストを壊さないこと。可能であれば再発防止のテストを `src/test/java` に追加する。
   - `refactoring`: 挙動を変えないことを最優先に、責務の分割・重複排除・マジックナンバーの定数化などを行う。
     既存のテストが通ることで「挙動が変わっていない」ことを保証する。
   - `documentation`: `README.md` や Javadoc など、指摘された記述の誤り・不足を修正する。
     実装コード（`src/main/java`）は変更しない。
3. 関連する既存 PR がないかを検索し、重複した対応を避ける。
4. 変更後、`./gradlew build` を実行してビルドとテストが通ることを確認する。
   - `documentation` ラベルのみで実装に変更がない場合はビルド確認を省略してよい。
5. プルリクエストを作成する。本文に `Closes #<issue番号>` を含め、Issue が自動クローズされるようにする。
6. 対応が難しく自動修正が適切でないと判断した場合（仕様が曖昧、影響範囲が大きすぎる、対象ラベルの意図と実際の内容が食い違う等）は、
