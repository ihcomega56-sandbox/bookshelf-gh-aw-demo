...
その Issue に `agent-fix` ラベルが付いている場合のみ、
...
3. 特定した Issue の現在ラベルを再取得し、`agent-fix` ラベルがない場合は何も変更せず終了する。
4. Issue のタイトル・本文・既存のコメント（トリアージ結果など）を読み、要求内容を正確に把握する。
5. `agent-fix` は自動修正の可否を示す追加ラベルであり、従来の分類ラベルと併用される。
...
- 対応するのは `agent-fix` ラベルが付いた Issue のみ。`bug` / `documentation` / `refactoring` を含む従来のラベルだけでは対応しない。
